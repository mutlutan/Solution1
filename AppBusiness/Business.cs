...
namespace AppBusiness
{
    ...
    public class Business : IDisposable
    {
        ...
        public MoResponse<object> SendPushNotify(object payload)
        {
            MoResponse<object> response = new();
            try
            {
                HttpClient http = new();
                string fcmServiceAccountFilename = $"{this.ContentRootPath}AppPushNotification.json";
#if DEBUG
                fcmServiceAccountFilename = $"{this.ContentRootPath}bin\\Debug\\net6.0\\AppPushNotification.json";
#endif
                var contents = System.IO.File.ReadAllTextAsync(fcmServiceAccountFilename).Result;
                ...
                var result = fcm.SendAsync(payload).Result;
        ...
        //Mobil Notification Jobı
        public MoResponse<object> SendMobileNotifyJob()
        {
            MoResponse<object> response = new();
            try
            {
                var listNotify = this.dataContext.MobilBildirim.Where(w => w.GonderildiMi == false && w.Tarih <= DateTime.Now).ToList();
                foreach (var rowNotify in listNotify)
                {
                    MobilBildirimLog mbLog = new();
                    List<object> listResponseData = new();

                    var payload = new List<object>();
                    var listMemberGroup = rowNotify.UyeGrupIds?.Split(',')?.Select(Int32.Parse)?.ToList();
                    if (listMemberGroup != null)
                    {
                        var listTopicName = this.dataContext.UyeGrup.Where(w => listMemberGroup.Contains(w.Id)).Select(s => s.Ad).ToList();
                        foreach (var topicName in listTopicName)
                        {
                            if (!string.IsNullOrEmpty(rowNotify.ResimUrl))
                            {
                                var msg = new
                                {
                                    message = new
                                    {
                                        topic = topicName.MyToSeoUrl(),
                                        notification = new
                                        {
                                            title = rowNotify.Baslik,
                                            body = rowNotify.Mesaj
                                        },
                                        android = new
                                        {
                                            notification = new
                                            {
                                                image = rowNotify.ResimUrl
                                            }
                                        },
                                        apns = new
                                        {
                    ...
                    var listMember = this.dataContext.MobilBildirimUye.Include(i => i.Uye).Where(w => w.MobilBildirimId == rowNotify.Id).ToList();
                    if (listMember.Count > 0)
                    {
                        foreach (var member in listMember)
                        {
                            int groupCount = listMemberGroup != null ? listMemberGroup.Where(w => w == member.Uye.UyeGrupId).Count() : 0;
                            if (groupCount == 0)
                            {
                                if (!string.IsNullOrEmpty(rowNotify.ResimUrl))
                                {
                                    var msg = new
                                    {
                                        message = new
                                        {
                                            token = member.Uye.FcmRegistrationToken,
                                            notification = new
                                            {
                                                title = rowNotify.Baslik,
                                                body = rowNotify.Mesaj
                    ...
                    {
                        var result = this.SendPushNotify(p);
                        if (result.Success)
                        {
                            listResponseData.Add(new { PayloadData = p, ResponseData = result.Data, ErrorMessages = new List<string>() });
                        }
                        else
                        {
                            listResponseData.Add(new { PayloadData = p, ResponseData = new { }, ErrorMessages = result.Message });
                        }
                    }
                    rowNotify.GonderildiMi = true;
                    rowNotify.GonderimTarihi = DateTime.Now;
                    this.dataContext.SaveChanges();

                    mbLog.Id = Guid.NewGuid();
                    mbLog.MobilBildirimId = rowNotify.Id;
                    mbLog.Durum = true;
                    mbLog.Tarih = DateTime.Now;
                    mbLog.MesajData = (new { rowNotify.UniqueId, rowNotify.Baslik, rowNotify.Mesaj, rowNotify.ResimUrl, rowNotify.Link }).MyObjToJsonText();
                    mbLog.ResponseData = listResponseData.MyObjToJsonText();
                    this.logDataContext.MobilBildirimLog.Add(mbLog);
                    this.logDataContext.SaveChanges();
                }
            }
            catch (Exception ex)
        ...
        //Sms Bildirim Jobı
        public MoResponse<object> SendSmsNotifyJob()
        {
            MoResponse<object> response = new();

            try
            {
                List<string> listGsmNo = new();

                var listNotify = this.dataContext.SmsBildirim.Where(w => w.GonderildiMi == false).ToList();
                foreach (var notify in listNotify)
                {
                    SmsLog smsLog = new();

                    int[] uyeGrupArray = notify.UyeGrupIds.Split(',').Select(n => Convert.ToInt32(n)).ToArray();
                    listGsmNo = this.dataContext.Uye.Where(w => w.UyeDurumId == 1 && uyeGrupArray.Contains(w.UyeGrupId)).Select(s => s.Gsm).ToList();

                    var listSmsBildirimUye = this.dataContext.SmsBildirimUye.Include(i => i.Uye).Where(w => w.SmsBildirimId == notify.Id).Select(s => s.Uye.Gsm).ToList();

                    listGsmNo.AddRange(listSmsBildirimUye);
                    listGsmNo = listGsmNo.Distinct().ToList();

                    List<MsgDatum> listMsgDatum = new();
                    listMsgDatum.Add(new MsgDatum
                    {
                        msg = notify.Mesaj,
                        tel = listGsmNo
                    });

                    var msgPaneliData = new MesajPaneliData
                    {
                        msgData = listMsgDatum,
                        start = (int)DateTime.UtcNow.Subtract(notify.Tarih).TotalSeconds,
                        tr = true
                    };
                    var smsResponse = this.SendSms(msgPaneliData);
                    if (smsResponse.Success)
                    {
                        notify.GonderildiMi = true;
                        notify.GonderimTarihi = DateTime.Now;
                        this.dataContext.SaveChanges();

                        smsLog.ResponseData = (new { RData = smsResponse.Data, ErrorMessage = new List<object>() }).MyObjToJsonText();
                    }
                    else
                    {
                        smsLog.ResponseData = (new { RData = new { }, ErrorMessage = smsResponse.Message }).MyObjToJsonText();
                    }

                    smsLog.Id = Guid.NewGuid();
                    smsLog.Durum = smsResponse.Success;
                    smsLog.SmsBildirimId = notify.Id;
                    smsLog.Tarih = DateTime.Now;
                    smsLog.MesajData = msgPaneliData.MyObjToJsonText();
                    this.logDataContext.SmsLog.Add(smsLog);
                    this.logDataContext.SaveChanges();
