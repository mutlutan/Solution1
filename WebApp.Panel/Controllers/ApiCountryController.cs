...
namespace WebApp.Panel.Controllers
{
    ...
    [ApiController]
    public class ApiCountryController : BaseController
    {
        public ApiCountryController(IServiceProvider _serviceProvider) : base(_serviceProvider) { }

        [HttpPost("Read")]
        [ResponseCache(Duration = 0)]
        [AuthenticateRequired(AuthorityKeys = "Country.D_R.")]
        ...
        [HttpGet("GetByNew")]
        [ResponseCache(Duration = 0)]
        [AuthenticateRequired(AuthorityKeys = "Country.D_C.")]
        public ActionResult GetByNew()
        {
             return Json(this.business.repository.RepCountry.GetByNew());
        }
        ...
        public ActionResult Create([FromBody] object obj)
        {
            ...
            try
            {
                var model = Newtonsoft.Json.JsonConvert.DeserializeObject<Country>(obj.MyToStr());
                if (model != null)
                {
                    Int32 id = this.business.repository.RepCountry.CreateOrUpdate(model, true);
                    this.business.repository.SaveChanges();
                    dsr.Data = this.business.repository.RepCountry.GetById(id);
                }
                else
                {
        ...
        [HttpPost("Update")]
        [ResponseCache(Duration = 0)]
        [AuthenticateRequired(AuthorityKeys = "Country.D_U.")]
        public ActionResult Update([FromBody] object obj)
        {
            ...
            try
            {
                ...
                if (model != null)
                {
                    Int32 id = this.business.repository.RepCountry.CreateOrUpdate(model, false);
                    this.business.repository.SaveChanges();
                    dsr.Data = this.business.repository.RepCountry.GetById(id);
                }
                else
        ...
        [HttpPost("Delete")]
        [ResponseCache(Duration = 0)]
        [AuthenticateRequired(AuthorityKeys = "Country.D_D.")]
        public ActionResult Delete([FromBody] object obj)
        {
            ...
            try
            {
                ...
                if (model != null)
                {
                    this.business.repository.RepCountry.Delete(model.Id);
                    this.business.repository.SaveChanges();
                }
                else
                {
